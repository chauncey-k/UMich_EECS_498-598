...
def compute_distances_two_loops(x_train: torch.Tensor, x_test: torch.Tensor):
    ...
    for i in range(num_train):
        for j in range(num_test):
            x_i = torch.flatten(x_train[i])
            x_j = torch.flatten(x_test[j])
            squared_diff = torch.square(x_i - x_j)
            dists[i][j] = torch.sum(squared_diff)
    ##########################################################################
    #                           END OF YOUR CODE                             #
    ##########################################################################
    return dists
...
def compute_distances_one_loop(x_train: torch.Tensor, x_test: torch.Tensor):
    ...
    for i in range(num_train):
        ...
        # Compute squared Euclidean distance between x_i and all test examples
        squared_diff = torch.square(x_i - x_test_flat)
        dists[i] = torch.sum(squared_diff, dim=1)
    ##########################################################################
    #                           END OF YOUR CODE                             #
    ##########################################################################
    return dists
...
def compute_distances_no_loops(x_train: torch.Tensor, x_test: torch.Tensor):
    ...
    x_train_square = torch.sum(x_train_flat ** 2, dim=1, keepdim=True)  # shape: (num_train, 1)
    x_test_square = torch.sum(x_test_flat ** 2, dim=1)  # shape: (num_test,)
    cross_term = torch.mm(x_train_flat, x_test_flat.t())  # shape: (num_train, num_test)
    dists = x_train_square - 2 * cross_term + x_test_square
...
def predict_labels(dists: torch.Tensor, y_train: torch.Tensor, k: int = 1):
    ...
    for j in range(num_test):
        # Find the indices of the closest k training examples to the j-th test example
        ...
        # Find the label with the highest count (in case of tie, choose the smallest label)
        predicted_label = torch.argmax(label_counts)

        # Assign the predicted label to y_pred[j]
        y_pred[j] = predicted_label
...
def knn_cross_validate(
    ...
    # Cross-validation loop over k values
    for k in k_choices:
        for fold in range(num_folds):
            ...
            y_val_pred = knn_classifier.predict(x_val_fold, k)

            # Compute accuracy and store it
            accuracy = torch.mean((y_val_pred == y_val_fold).float())
...
def knn_get_best_k(k_to_accuracies: Dict[int, List]):
    ...
    best_accuracy = -1.0  # Initialize to a very low value
